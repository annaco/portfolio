<?php
  include 'components/header/header.php';
  include 'components/menu/menu.php';
?>

<div class="wrapper-title">Mina projekt</div>


<div class="project">
  <div class="project-name">  
      Klickspel
  </div>  
  <div class="project-box">
    <div class="project-left  fade-right">
      <a href="http://klickspel.carinnojd.se/"  target="_blank">
        <img class="project-image" src="image/klickspel.png" alt="Klickspel">
      </a> 
    </div>
    <div class="project-right project-text fade-left">
      <div class="project-description">
        Ett grupparbete vi gjorde i kursen javascript.
        Vi skulle skapa ett spel och då valde vi att göra detta barnspel.
        <div class="project-technics">
          <div class="project-technics__technic"> jQuery</div>
          <div class="project-technics__technic"> HTML</div>
          <div class="project-technics__technic"> CSS</div>
        </div>
      </div>
    </div>
  </div>
</div> 




<div class="project">
  <div class="project-name">
     Portfolio wordpress
  </div>
  <div class="project-box">
     <div class="project-left project-text fade-right">
        I javascripts kursen skulle vi lära oss använda javascript i kombination med wordpress och då skapade jag en portfolio.
        <div class="project-technics">
          <div class="project-technics__technic"> jQuery</div>
          <div class="project-technics__technic"> PHP</div>
          <div class="project-technics__technic"> Wordpress</div>
        </div>
     </div>
     <div class="project-right fade-left">
       <a href="../Medieinstitutet/Javascript/Portfolio/wordpress/index.php"  target="_blank">
         <img class="project-image" src="image/portfolio.png" alt="Wordpress portfolio">
       </a> 
     </div>
    </div>
 </div>   

<div class="project">
  <div class="project-name">
    Annas trädgård
  </div>
  <div class="project-box">
    <div class="project-left  fade-right">
      <a href="http://annastradgard.com"  target="_blank">
        <img class="project-image" src="image/annastradgard.png" alt="Annas Trädgård">
      </a>
    </div>
    <div class="project-right project-text fade-left">
      Mitt exjobb som jag nu håller på att utveckla. 
      <div class="project-technics">
          <div class="project-technics__technic"> PHP</div>
          <div class="project-technics__technic"> jQuery</div>
          <div class="project-technics__technic"> HTML</div>
          <div class="project-technics__technic"> CSS</div>
          <div class="project-technics__technic"> MySQL</div>
          <div class="project-technics__technic"> SQL</div>
        </div>
    </div>
  </div>
</div>

<div class="project">
   <div class="project-name">
    Webbloppis
   </div>
   <div class="project-box">
     <div class="project-left project-text fade-right">
       Ett grupparbete i php-kursen. Vi skulle göra en sida som vi sedan skulle kunna tjäna pengar på. 
       Vi valde att göra en loppmarknad.
       <div class="project-technics">
          <div class="project-technics__technic"> PHP</div>
          <div class="project-technics__technic"> HTML</div>
          <div class="project-technics__technic"> CSS</div>
          <div class="project-technics__technic"> MySQL</div>
          <div class="project-technics__technic"> SQL</div>
        </div>
     </div>
     <div class="project-right">
      <a href="../Medieinstitutet/php/Loppis/index.php"  target="_blank">
        <img class="project-image" src="image/webbloppis.png" alt="Webbloppis">
      </a>
     </div>
   </div>
</div>  


<?php
include 'components/footer/footer.php';
?>